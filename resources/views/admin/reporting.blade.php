...
@section('content')
    ...
    <div class="stats-grid" style="grid-template-columns: repeat(4, 1fr); gap: 16px; margin-bottom: 24px;">
        <div class="stat-card" style="padding: 16px; min-height: auto;">
            ...
            <div class="number" style="font-size: 24px; margin-bottom: 4px;">{{ $stats['total'] }}</div>
            <div class="trend" style="font-size: 12px;">Toutes périodes</div>
        </div>
        <div class="stat-card" style="padding: 16px; min-height: auto;">
            <h3 style="font-size: 14px; margin-bottom: 8px;">Taux de résolution</h3>
            <div class="number" style="font-size: 24px; margin-bottom: 4px;">{{ $stats['total'] > 0 ? round(($stats['cloturees'] / $stats['total']) * 100, 1) : 0 }}%</div>
    ...
    <script>
        ...
        new Chart(statusCtx, {
            ...
            data: {
                ...
                datasets: [{
                    data: [
                        {{ $stats['total'] - $stats['en_cours'] - $stats['a_risque'] - $stats['cloturees'] }},
                        {{ $stats['en_cours'] }},
                        {{ $stats['a_risque'] }},
                        {{ $stats['cloturees'] }}
                    ],
        ...
        new Chart(categoryCtx, {
            type: 'bar',
            data: {
                labels: {!! json_encode($stats['par_categorie']->keys()) !!},
                datasets: [{
                    label: 'Nombre de demandes',
                    data: {!! json_encode($stats['par_categorie']->values()) !!},
        ...
        const societeData = {!! json_encode($stats['par_societe']->toArray()) !!};
