@extends(auth()->check() && auth()->user()->type == 1 ? 'layouts.appEmployer' : 'layouts.appAdministration')
    ...
    <section class="demande-details" id="section-details">
      <div class="details-container">
        ...
        <!-- Informations principales 
            ...
            <span class="reference">Référence: TK-{{ str_pad($demande->id, 5, '0', STR_PAD_LEFT) }}</span>
            <span class="date">Créée le: {{ $demande->formatted_created_at }}</span>
            <span class="status status-{{ $demande->statut }}">{{ ucfirst($demande->statut) }}</span>
          </div>-->
        ...
        <div class="demande-grid">
          ...
          <div class="demande-info">
            ...
            <div class="info-group">
              ...
              <span class="priorite-{{ strtolower($demande->priorite) }}">{{ $demande->priorite }}</span>
            </div>
            
            <div class="info-group">
              <label>Date limite:</label>
              <span>{{ $demande->date_limite ? $demande->date_limite->format('d/m/Y') : 'Non définie' }}</span>
            </div>
              ...
              <div class="progress-container">
                ...
                <div class="progress-bar-container">
                  <div class="progress-bar">
                    <div class="progress {{ $statuses[$currentStatus]['class'] ?? 'status-pending' }}" style="width: {{ $progress }}%;"></div>
                  </div>
                  <div class="progress-percentage">{{ round($progress) }}%</div>
                </div>
                
                <div class="current-status">
                  <span class="status-badge {{ $statuses[$currentStatus]['class'] ?? 'status-pending' }}">
                    {{ $statuses[$currentStatus]['icon'] ?? '⏳' }} Statut actuel: {{ ucfirst($currentStatus) }}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div class="pieces-jointes">
            ...
            @if($demande->fichier)
              <div class="fichier-item">
                  <a href="#" onclick="openFileModal('{{ asset('storage/' . $demande->fichier) }}')" class="file-preview-link">
            ...
            @if($demande->piecesJointes && $demande->piecesJointes->count())
              @foreach($demande->piecesJointes as $pj)
                <div class="fichier-item">
                    <a href="#" onclick="openFileModal('{{ asset('storage/' . $pj->chemin) }}')" class="file-preview-link">
                        📎 {{ $pj->nom_fichier }}
                    </a>
                </div>
              @endforeach
          ...
          @if(auth()->user()->type == 0) {{-- Admin --}}
            <div class="admin-actions">
              ...
              <div class="action-buttons">
                <button class="gida-btn gida-btn-primary" onclick="window.location.href='{{ route('demandes.edit', $demande->id) }}'">
                  Modifier le statut
                </button>
